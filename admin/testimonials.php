...
$eid=intval($_GET['eid']);
$status="0";
$sql = "UPDATE neorent_testimonials_table SET status=:status WHERE  id=:eid";
...
$sql = "UPDATE neorent_testimonials_table SET status=:status WHERE  id=:aeid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
						...
						<div class="panel panel-default">
							...
							<?php if($error){?><div class="errorWrap"><strong>ERROR</strong>:<?php echo htmlentities($error); ?> </div><?php } 
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
									...
									<?php $sql = "SELECT neorent_users_table.FullName,neorent_testimonials_table.UserEmail,neorent_testimonials_table.Testimonial,neorent_testimonials_table.PostingDate,neorent_testimonials_table.status,neorent_testimonials_table.id from neorent_testimonials_table join neorent_users_table on neorent_users_table.Emailid=neorent_testimonials_table.UserEmail";
$query = $dbh -> prepare($sql);
$query->execute();
